...
<div class="modal fade" id="editProductos{{ $data->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <!-- Encabezado del modal -->
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="exampleModalLabel">Modificación de producto {{ $data->nombre }}</h1>
            ...
            <div class="modal-body">
                ...
                <form action="{{ route('productos.update', ['producto' => $data->id]) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="categoria" name="categoria" value="{{$data->categoria}}">
                    </div>
                    <div class="mb-3">
                        <label for="tamanio" class="col-form-label">Tamaño:</label>
                        <input type="text" class="form-control" id="tamanio" name="tamanio" value="{{$data->tamanio}}">
                    </div>

                    <select class="form-select" name="estado" aria-label="Default select example">
                        <option selected>Estado del producto</option>
                        <option value="1" {{$data->estado == 1 ? 'selected' : ''}}>Activo</option>
                        <option value="2" {{$data->estado == 2 ? 'selected' : ''}}>Inactivo</option>
                    </select>
                    <div class="mb-3">
